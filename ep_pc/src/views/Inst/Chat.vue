...
export default {
    async mounted() {
        const res = await this.$http.get(`/question/list/${this.userInfo.instId}?answered=0`)
        const res2 = await this.$http.get(`/question/list/${this.userInfo.instId}?answered=1`)
    ...
    methods: {
        ...
        handleAnswer(row) {
            this.answerForm.instId = this.userInfo.instId
